...
-spec set_header(request(JHM), binary(), binary()) -> request(JHM).
...
-spec set_query(request(JII), list({binary(), binary()})) -> request(JII).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(JIM), gleam@http:method()) -> request(JIM).
...
-spec set_scheme(request(JIT), gleam@http:scheme()) -> request(JIT).
set_scheme(Req, Scheme) ->
    erlang:setelement(5, Req, Scheme).

-spec set_host(request(JIW), binary()) -> request(JIW).
set_host(Req, Host) ->
    erlang:setelement(6, Req, Host).

-spec set_port(request(JIZ), integer()) -> request(JIZ).
set_port(Req, Port) ->
    erlang:setelement(7, Req, {some, Port}).

-spec set_path(request(JJC), binary()) -> request(JJC).
set_path(Req, Path) ->
    erlang:setelement(8, Req, Path).

-spec set_cookie(request(JJF), binary(), binary()) -> request(JJF).
