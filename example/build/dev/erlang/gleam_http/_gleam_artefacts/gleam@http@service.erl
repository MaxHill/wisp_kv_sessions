...
-spec map_response_body(
    fun((JST) -> gleam@http@response:response(JSF)),
    fun((JSF) -> JSG)
) -> fun((JST) -> gleam@http@response:response(JSG)).
...
-spec prepend_response_header(
    fun((JSX) -> gleam@http@response:response(JSZ)),
    binary(),
    binary()
) -> fun((JSX) -> gleam@http@response:response(JSZ)).
...
-spec ensure_post(gleam@http@request:request(JSK)) -> {ok,
        gleam@http@request:request(JSK)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(JUG)) -> JUO)) -> fun((gleam@http@request:request(JUG)) -> JUO).
