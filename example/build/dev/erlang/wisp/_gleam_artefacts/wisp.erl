...
-spec method_override(gleam@http@request:request(SAA)) -> gleam@http@request:request(SAA).
...
-spec fn_with_bad_request_error(fun((SBB) -> {ok, SBC} | {error, any()})) -> fun((SBB) -> {ok,
        SBC} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((SAV, bitstring()) -> {ok, SAV} |
        {error, gleam@http@response:response(body())}),
    SAV
) -> {ok, {gleam@option:option(buffered_reader()), integer(), SAV}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), SBT})) -> list({binary(), SBT}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, SBW} | {error, any()},
    fun((SBW) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, SAN} | {error, any()}) -> {ok, SAN} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
