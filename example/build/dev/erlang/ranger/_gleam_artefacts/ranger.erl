...
-spec create(
    ...
    fun((FSX) -> FSX),
    fun((FSW, FSX) -> FSW),
    fun((FSW, FSW) -> gleam@order:order())
) -> fun((FSW, FSW, FSX) -> {ok, gleam@iterator:iterator(FSW)} | {error, nil}).
...
-spec create_infinite(
    fun((FTB) -> boolean()),
    fun((FTB, FTC) -> FTB),
    fun((FTB, FTB) -> gleam@order:order())
) -> fun((FTB, FTC) -> {ok, gleam@iterator:iterator(FTB)} | {error, nil}).
