...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JVF} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JVF} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JVB} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JVB} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JUX} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JUX} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(JVL), fun((JVL) -> json())) -> json().
...
-spec array(list(JVP), fun((JVP) -> json())) -> json().
