        ...
        {ok, {Cookies_string, Headers}} ->
            New_cookie_string = begin
                ...
                _pipe@2 = gleam@list:filter(
                    _pipe,
                    fun(Str) -> _pipe@1 = gleam@string:starts_with(Str, Name),
                        gleam@bool:negate(_pipe@1) end
                ),
...
get_session_id(Cookie_name, Req) ->
    gleam@io:println(<<"get_session_id"/utf8>>),
    gleam@io:debug(Cookie_name),
    gleam@io:debug(Req),
    T = wisp:get_cookie(Req, Cookie_name, signed),
    _ = gleam@io:debug(T),
