...
-type message(LSB) :: {internal, internal_message()} | {user, LSB}.

-type loop_message(LSC) :: {packet, bitstring()} | {custom, LSC}.

-type loop_state(LSD, LSE) :: {loop_state,
        {ok, {glisten@transport:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(LSD)),
        glisten@transport:transport(),
        LSE}.

-type connection(LSF) :: {connection,
        {ok, {glisten@transport:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(LSF))}.

-type handler(LSG, LSH) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(LSG), LSH, connection(LSG)) -> gleam@otp@actor:next(loop_message(LSG), LSH)),
        fun((connection(LSG)) -> {LSH,
            gleam@option:option(gleam@erlang@process:selector(LSG))}),
        gleam@option:option(fun((LSH) -> nil)),
        glisten@transport:transport()}.

-spec start(handler(LSR, any())) -> {ok,
        gleam@erlang@process:subject(message(LSR))} |
