...
-spec equal(LCV, LCV) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(LCW, LCW) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, LCX} | {error, any()}) -> LCX.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, LDC}) -> LDC.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(LDF)) -> LDF.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
