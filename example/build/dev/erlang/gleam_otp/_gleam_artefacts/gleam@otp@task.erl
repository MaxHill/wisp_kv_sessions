...
-opaque task(KYT) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(KYT))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(KYU) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, KYU}.

-spec async(fun(() -> KYV)) -> task(KYV).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(KYZ), integer()) -> {ok, KYZ} | {error, await_error()}.
...
-spec await(task(KZD), integer()) -> KZD.
await(Task, Timeout) ->
...
-spec try_await_forever(task(KZF)) -> {ok, KZF} | {error, await_error()}.
...
-spec await_forever(task(KZJ)) -> KZJ.
await_forever(Task) ->
