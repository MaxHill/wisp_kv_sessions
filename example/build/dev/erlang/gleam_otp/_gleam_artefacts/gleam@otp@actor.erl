...
-type message(JXH) :: {message, JXH} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(JXI, JXJ) :: {continue,
        JXJ,
        gleam@option:option(gleam@erlang@process:selector(JXI))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(JXK, JXL) :: {ready, JXK, gleam@erlang@process:selector(JXL)} |
    {failed, binary()}.

-type self(JXM, JXN) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        JXM,
        gleam@erlang@process:subject(JXN),
        gleam@erlang@process:selector(message(JXN)),
        gleam@otp@system:debug_state(),
        fun((JXN, JXM) -> next(JXN, JXM))}.

-type spec(JXO, JXP) :: {spec,
        fun(() -> init_result(JXO, JXP)),
        integer(),
        fun((JXP, JXO) -> next(JXP, JXO))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(JXQ) :: {ack,
        {ok, gleam@erlang@process:subject(JXQ)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-spec continue(JXX) -> next(any(), JXX).
continue(State) ->
    {continue, State, none}.

-spec with_selector(next(JYB, JYC), gleam@erlang@process:selector(JYB)) -> next(JYB, JYC).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(JYN))) -> gleam@erlang@process:selector(message(JYN)).
...
-spec receive_message(self(any(), JYJ)) -> message(JYJ).
...
-spec init_selector(
    gleam@erlang@process:subject(KDB),
    gleam@erlang@process:selector(KDB)
) -> gleam@erlang@process:selector(message(KDB)).
...
-spec initialise_actor(
    spec(any(), JZE),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(JZE)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), JZR)) -> {ok, gleam@erlang@process:subject(JZR)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(JZX, fun((JZY, JZX) -> next(JZY, JZX))) -> {ok,
        gleam@erlang@process:subject(JZY)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(KAE), KAE) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-spec call(
    gleam@erlang@process:subject(KAG),
    fun((gleam@erlang@process:subject(KAI)) -> KAG),
    integer()
) -> KAI.
