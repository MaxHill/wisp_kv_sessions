...
-type spec(KLX, KLY) :: {spec,
        KLX,
        integer(),
        integer(),
        fun((children(KLX)) -> children(KLY))}.

-opaque children(KLZ) :: {ready, starter(KLZ)} | {failed, child_start_error()}.

-opaque child_spec(KMA, KMB, KMC) :: {child_spec,
        fun((KMB) -> {ok, gleam@erlang@process:subject(KMA)} |
            {error, gleam@otp@actor:start_error()}),
        fun((KMB, gleam@erlang@process:subject(KMA)) -> KMC)}.
...
-spec start_child(child_spec(any(), KMJ, KMK), KMJ) -> {ok, child(KMK)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    KMX,
    instruction(),
    child_spec(any(), KMX, KMZ),
    child(KMZ)
) -> {ok, {child(KMZ), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(KNQ), child_spec(any(), KNQ, KNT)) -> children(KNT).
...
-spec add(children(KNY), child_spec(any(), KNY, KOB)) -> children(KOB).
...
-spec supervisor(
    fun((KOG) -> {ok, gleam@erlang@process:subject(KOH)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KOH, KOG, KOG).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec worker(
    fun((KOO) -> {ok, gleam@erlang@process:subject(KOP)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KOP, KOO, KOO).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec returning(
    child_spec(KOW, KOX, any()),
    fun((KOX, gleam@erlang@process:subject(KOW)) -> KPD)
) -> child_spec(KOW, KOX, KPD).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-spec init(spec(any(), KPI)) -> gleam@otp@actor:init_result(state(KPI), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(KPO)) -> gleam@otp@actor:next(message(), state(KPO)).
...
-spec loop(message(), state(KPT)) -> gleam@otp@actor:next(message(), state(KPT)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
