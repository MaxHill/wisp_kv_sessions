...
-opaque subject(IGG) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, IGG}.

-type do_not_leak() :: any().

-type selector(IGH) :: any() | {gleam_phantom, IGH}.
...
-type call_error(IGI) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, IGI}.
...
-spec select(selector(IGZ), integer()) -> {ok, IGZ} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-spec select_forever(selector(IHD)) -> IHD.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-spec map_selector(selector(IHF), fun((IHF) -> IHH)) -> selector(IHH).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-spec merge_selector(selector(IHJ), selector(IHJ)) -> selector(IHJ).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(IHN), fun((exit_message()) -> IHN)) -> selector(IHN).
...
-spec selecting(selector(IHQ), subject(IHS), fun((IHS) -> IHQ)) -> selector(IHQ).
...
-spec 'receive'(subject(IGT), integer()) -> {ok, IGT} | {error, nil}.
...
-spec selecting_record2(
    selector(IHV),
    any(),
    fun((gleam@dynamic:dynamic_()) -> IHV)
) -> selector(IHV).
...
-spec selecting_record3(
    selector(IHZ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IHZ)
) -> selector(IHZ).
...
-spec selecting_record4(
    selector(IID),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IID)
) -> selector(IID).
...
-spec selecting_record5(
    selector(IIH),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIH)
) -> selector(IIH).
...
-spec selecting_record6(
    selector(IIL),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIL)
) -> selector(IIL).
...
-spec selecting_record7(
    selector(IIP),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIP)
) -> selector(IIP).
...
-spec selecting_record8(
    selector(IIT),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIT)
) -> selector(IIT).
...
-spec selecting_anything(selector(IIX), fun((gleam@dynamic:dynamic_()) -> IIX)) -> selector(IIX).
...
-spec try_call(subject(IJI), fun((subject(IJK)) -> IJI), integer()) -> {ok, IJK} |
    {error, call_error(IJK)}.
...
-spec call(subject(IJP), fun((subject(IJR)) -> IJP), integer()) -> IJR.
...
-spec send_after(subject(IJU), integer(), IJU) -> timer().
