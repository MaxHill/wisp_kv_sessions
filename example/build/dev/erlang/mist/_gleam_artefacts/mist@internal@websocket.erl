...
-type valid_message(QPU) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, QPU}.

-type websocket_message(QPV) :: {valid, valid_message(QPV)} | invalid.
...
-type handler_message(QPW) :: {internal, gramps@websocket:frame()} | {user, QPW}.

-type websocket_state(QPX) :: {websocket_state,
        bitstring(),
        QPX,
...
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(QRF))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(QRF))).
...
-spec apply_frames(
    ...
    fun((QQV, websocket_connection(), handler_message(QQW)) -> gleam@otp@actor:next(QQW, QQV)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(QQW), QQV),
    fun((QQV) -> nil)
) -> gleam@otp@actor:next(websocket_message(QQW), QQV).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {QQG,
        gleam@option:option(gleam@erlang@process:selector(QQH))}),
    fun((QQG) -> nil),
    fun((QQG, websocket_connection(), handler_message(QQH)) -> gleam@otp@actor:next(QQH, QQG)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(QQH))} | {error, nil}.
