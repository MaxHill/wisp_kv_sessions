...
-opaque builder(RCP, RCQ) :: {builder,
        integer(),
        fun((gleam@http@request:request(RCP)) -> gleam@http@response:response(RCQ)),
...
-type websocket_message(RCR) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, RCR}.
...
-spec new(
    fun((gleam@http@request:request(RDO)) -> gleam@http@response:response(RDQ))
) -> builder(RDO, RDQ).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(RDU, RDV), integer()) -> builder(RDU, RDV).
port(Builder, Port) ->
    erlang:setelement(2, Builder, Port).

-spec read_request_body(
    builder(bitstring(), REA),
    integer(),
    gleam@http@response:response(REA)
) -> builder(mist@internal@http:connection(), REA).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(RFH)
) -> {ok, websocket_message(RFH)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((RFN, mist@internal@websocket:websocket_connection(), websocket_message(RFO)) -> gleam@otp@actor:next(RFO, RFN)),
    fun((mist@internal@websocket:websocket_connection()) -> {RFN,
        gleam@option:option(gleam@erlang@process:selector(RFO))}),
    fun((RFN) -> nil)
...
-spec server_sent_events(
    ...
    fun(() -> gleam@otp@actor:init_result(RGC, RGD)),
    fun((RGD, sse_connection(), RGC) -> gleam@otp@actor:next(RGD, RGC))
) -> gleam@http@response:response(response_data()).
